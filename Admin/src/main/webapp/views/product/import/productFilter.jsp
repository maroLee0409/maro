...
window.onload = function() {
    const filters = [
        ...
        { checkboxId: 'priceCheckbox', filterId: 'priceFilterItem', value: '${filter.minPrice}${filter.maxPrice}' },
	...
	<div class="search-container">
	    <form action="productSearchList.do" method="get">
	        ...
	        <div id="filterContainer" class="filter-container">
	            ...
	            <div class="filter-item">
	                <label><input type="checkbox" id="userNoCheckbox" onclick="toggleFilterItem(this, 'userNoFilterItem')"> 유저 번호</label>
	            </div>
	            <div class="filter-item">
	                <label><input type="checkbox" id="viewsCheckbox" onclick="toggleFilterItem(this, 'viewsFilterItem')"> 조회수</label>
	            </div>
	            <div class="filter-item">
	                <label><input type="checkbox" id="priceCheckbox" onclick="toggleFilterItem(this, 'priceFilterItem')"> 상품 가격</label>
	            </div>
	            <div class="filter-item">
	            ...
	            <div id="categoryFilterItem" class="filter-item" style="display:none;">
	                <label for="categoryFilter" class="filter-label filter-category-label">카테고리 번호</label>
	                <select id="categoryFilter" name="categoryNo" class="filter-select">
	                    <option value="">전체</option>
	                    <c:forEach var="category" items="${categoryList}">
	                        <option value="${category.getCategory_No()}" <c:if test="${category.getCategory_No() == filter.categoryNo}">selected</c:if>>${category.name}</option>
	                    </c:forEach>
	                </select>
	            </div>
	            ...
	            <div id="priceFilterItem" class="filter-item" style="display:none;">
	                <label for="minPriceFilter" class="filter-label">최소 상품 가격</label>
	                <input type="number" id="minPriceFilter" name="minPrice" class="filter-input" placeholder="최소 상품 가격" value="${filter.minPrice}">
	                <label for="maxPriceFilter" class="filter-label">최대 상품 가격</label>
	                <input type="number" id="maxPriceFilter" name="maxPrice" class="filter-input" placeholder="최대 상품 가격" value="${filter.maxPrice}">
	            </div>
	            ...
	            <div id="dateFilterItem" class="filter-item" style="display:none;">
	                <label for="startDateFilter" class="filter-label">상품 등록일 시작</label>
	                <input type="date" id="startDateFilter" name="startDate" class="filter-input" value="${filter.startDate}">
	                <label for="endDateFilter" class="filter-label">상품 등록일 종료</label>
